$(document).ready(function() {
    ...
    function updatepostId() {
        ...
        xhr.open("GET", "http://127.0.0.1:3000/api/post/" + post_id);
    ...
    function createpost() {
        ...
        xhr.open("PUT", "http://127.0.0.1:3000/api/post/" + post_id);
        xhr.setRequestHeader("content-type", "application/json");
        xhr.setRequestHeader("cache-control", "no-cache");
    ...
    function deletepost() {
        ...
        xhr.open("DELETE", "http://127.0.0.1:3000/api/post/" + post_id);
        xhr.setRequestHeader("content-type", "application/json");
        xhr.setRequestHeader("cache-control", "no-cache");
    ...
    xhr.addEventListener("readystatechange", function() {
        if (this.readyState === 4) {
            ...
            for (var i = 0; i <= data.length - 1; i++) {
                ...
                str += '<div class="item ' + data[i]._id + ' ' + data[i].title + ' ' + data[i].des + '">';
    ...
    xhr.open("GET", "http://127.0.0.1:3000/api/post/");
    xhr.setRequestHeader("cache-control", "no-cache");
    xhr.send();
    ...
    function get_post_image() {
        var settings = {
            "async": true,
            "crossDomain": true,
            "url": "http://127.0.0.1:3000/api/upload/" + post_id,
            "method": "GET",
            "cache": false
        };
        ...
        $.ajax(settings).done(function(response) {
            ...
            $.each(obj, function(index, value) {
                ...
                var str = '';
                str += '<img class="item ' + value.name + ' ' + value._id + '" src="../uploads/' + value.name + '">';
    ...
    function get_all_image() {
        ...
        $.ajax(settings).done(function(response) {
            ...
            $.each(obj, function(index, value) {
                ...
                var str = '';
                str += '<div class="item ' + value.name + ' ' + value._id + '" style="background-image:url(../uploads/' + value.name + ')">';
                str += '</div>';
